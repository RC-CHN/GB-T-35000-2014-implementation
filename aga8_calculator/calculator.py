...
def _calculate_P_internal(pm, T, B_calc, SUM1, K0, G0, Q0, F0, U0):
    ...
    pr_k = pr ** k[n_range]
    term_vec = (
        (b[n_range] - c[n_range] * k[n_range] * pr_k)
        * (pr ** b[n_range])
        * np.exp(-c[n_range] * pr_k)
    )
...
def calculate_z_factor_bisection(
    T, P0, x, max_iterations=1000, tolerance=0.00001, log_callback=None
):
    """
    使用二分法计算天然气压缩因子Z。
    """
    ...
    Cn_vec_sum1 = (
        a[n_range_sum1]
        * ((G0 + 1 - g[n_range_sum1]) ** g[n_range_sum1])
    ...
    if not (P_low < P0 < P_high) and log_callback:
        log_callback(
            f"警告: 目标压力 P0={P0} 不在初始搜索区间 [{P_low:.4f}, {P_high:.4f}] 内。\n"
        )
...
if __name__ == "__main__":
    ...
    calculate_z_factor_bisection(
        T_in, P0_in, x_in, tolerance=0.00001, log_callback=print
    )
