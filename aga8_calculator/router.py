...
class CalculationRequest(BaseModel):
    base_components: Dict[str, float] = Field(
        ..., example={"Methane": 0.9, "Nitrogen": 0.05, "Ethane": 0.05}
    )
...
def adjust_compositions_with_hydrogen(
    base_components: Dict[str, float], hydrogen_fraction: float
) -> Dict[str, float]:
...
def calculate(request: CalculationRequest):
    ...
    for api_name, fraction in final_components_api_names.items():
        ...
        if index is None:
            raise HTTPException(
                status_code=500,
                detail=f"内部错误: 组分'{internal_name}'在顺序列表中未找到。",
            )
