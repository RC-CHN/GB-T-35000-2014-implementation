...
def _cv_integrand(T: float, rho: float, x: Dict[str, float], eos: AGA8EOS, dT_rel: float) -> float:
    """计算积分项"""
...
def cv_real(T: float, p: float, x: Dict[str, float], eos: AGA8EOS,
            n_int_steps: int = 80, dT_rel: float = 1e-3):
    """计算 cv，返回 cv, rho, Z"""
...
def sound_speed(T: float, p: float, x: Dict[str, float], M: float, eos: AGA8EOS,
                n_int_steps: int = 80, dT_rel: float = 1e-3) -> float:
    """计算声速 W"""
    cp, cv, rho, Z, dZdT, dZdrho = cv_real(T, p, x, eos,
