...
def _cv_integrand(T, rho_molar, x, eos, dT_rel):
    ...
    # 二阶导同样数值差分
    d2ZdT2 = (eos.Z(T+dT, rho_molar) - 2*eos.Z(T, rho_molar) + eos.Z(T-dT, rho_molar)) / (dT*dT)
...
if __name__ == "__main__":
    ...
    print(f"{'Property':<20} | {'Reference':<15} | {'Calculated (CP)':<18} | {'Error %':<10}")
    print("-" * 85)
    ...
    print(f"Error              : {abs(w_manual - ref['speed_sound'])/ref['speed_sound']*100:.4f} %")
